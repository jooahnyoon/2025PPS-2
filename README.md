...
400의 배수인수는 4의 배수이기에, 먼저 400으로 나누었을때 나머지가 0인지를 판별하여 
성립하면 1을 반환한다. 100으로 나누었을때 나머지가 0인수는 무조건 윤년이 아니기에0을 반환한다. 
100의 배수가 아니지만 4로 나누었을때 나머지가 0인 년도는 윤년이기에 4로 나눈 나머지가 0이면 
1을 반환하고 그렇지 않은 경우에는 0을 반환한다.  

## A188 사분면 고르기
![A188](./captures/A188.png) 
x, y값을 입력받아 if 둘다 0보다 크면 1 출력
x가 0보다 작고, y가 0보다 크면 2출력
x와 y모두 0보다 작으면 3출력
x는 0보다 크고, y는 0보다 작으면 4출력
그 외의 경우에는 wrong number 출력한다. 

## A189 알람 시계
![A189](./captures/A189.png) 
일단 m에다가 45를 뺀다. 
만약 m이 0보다 작아지면 h를 1감소하고, m에다가 60을 더하면 된다.
만약 h이 0보다 작아지면 h에 23을 대입하면 된다. 

## A190 주사위 세개
![A190](./captures/A190.png) 
주사위의 값 3개를 비교한다. 
1) 세개 모두 같은 경우
a1과 a2가 같고, a2와 a3가 같을 때 10000 + (a1 * 1000)을 출력
2) 두개가 같은 경우
a1과 a2가 같지만 a2와 a3가 같지 않은 경우, a1과 a3가 같은 경우
1000 + (a1 * 100)을 출력
 a2와 a3가 같은 경우
1000 + (a2 * 100)을 출력한다. 
3) 모두 같지 않은 경우
max = 0으로 두고, a1과 a2를 비교하여 더 큰 값을 max에 대입한다. 
...
바구니 갯수 n, 공 바꾸기 횟수 m을 입력받고, 제한조건을 충족하기위한 while문 실행
처음에는 바구니의 번호와 공의 번호가 동일하기에 arr[i] = i를 대입하여 n이 될때까지 반복한다.
바구니의 번호를 입력받고, 함수를 실행하는데, 함수에서는 int temp를 만들어서 둘의 array 값을 
서로 바꿔준다. m번 실행후 최종적으로 i는 1부터 n까지 arr[i]를 출력한다. 

## A199 별 찍기 - 20
![A199](./captures/A199.png) 
n을 입력받고, 가로 세로 모두 1씩 증가하며 n번 반복시행
내부 for문에서는 "* "를 출력하며 
if i가 짝수일때는 " "를 앞에 출력하여 한칸 들어가도록 한다. 

## A200 돌 게임 2
![A200](./captures/A200.png)
항상 상근이가 먼저 시작하고 둘다 1 또는 3개만 가져갈 수 있기에
완벽하게 본인이 이기는 수를 둔다는 가정하에,
원리는 간단하다 n이 홀수면 창영이가 이기고, n이 짝수면 상근이가 이긴다.  

## A201 단어 뒤집기 2
![A201](./captures/A201.png) 
'<'를 만나면, <가 시작되기 전에 쌓인 단어가 있으면 뒤집는다. 
tag를 true로 하고 '<'를 출력한다. 
'>'를 만나면, tag를 false로 하고 '>'를 출력한다. 
tag가 true인 상황 즉 tag의 내부인 경우 그대로 출력한다.
그외의 경우 즉, <>밖의 경우 stack에 쌓인 단어를 거꾸로 출력. 
else인 경우 단어 push. 문자열이 끝났는데 stack에 남아있는 단어 출력. 

## A213 포도주 시식
![A213](./captures/A213.png) 
아이디어는 1번잔부터 2개씩 묶어서 한칸씩 띄워서 묶은 값의 합을 구하고 
2번잔부터 2개씩 묶어서 한칸씩 띄워서 묶은 값의 합을 구해서
서로의 값을 비교해 더 큰 값을 출력한다. 
for반복문으로 arr[i]를 입력받고 i % 3 == 0 즉 3의 배수인지를 판별,
for반복문으로 j-2를 3으로 나눈 나머지가 0일때 즉 등차수열 2 5 8 ... 이 되는 경우를 판별하여
해당하는 경우 continue하고 그렇지 않은 경우에는 더한다. 
합한 값을 서로 비교하여 더 큰 값을 출력한다. 

## A218 할로윈의 사탕
![A218](./captures/A218.png) 
아이디어는 사탕 갯수 c를 형제수 v로 나눈 값을 a[i]에 저장하고 
나머지 값을 b[i]에 저장한 후 a[i], b[i]를 조건에 맞춰서 n번 출력하면 된다. 

## A219 TV 크기
![A219](./captures/A219.png) 
비례이기에 k라는 비례상수를 사용해서 실제 높이와 너비 값은 
k * h, k * w가 된다. 
피타고라스 정리에 의하여 (k*h)의 제곱 + (k*w)의 제곱은 대각선의 제곱이 된다. 
대각선의 길이는 (k*h)의 제곱 + (k*w)의 제곱을 sqrt한 것이고
대각선의 길이, h, w의 값은 주어지기에 이 과정을 통해 k의 값을 구한다. 
k * h, k*w를 출력하되 floor를 사용하여 소수점 이하를 버린다. 

## A220 학점계산
![A220](./captures/A220.png) 
<string, double>를 쌍으로 묶는 map grade를 생성한다. 
각각의 grade에 A+부터 F까지의 값을 집어넣는다. 
cin으로 n을 입력받고, grade[n]을 fixed setprecision(1)을 사용하여 
소수점 첫째자리까지 출력한다. 